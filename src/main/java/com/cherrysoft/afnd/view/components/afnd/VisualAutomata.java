...
import com.cherrysoft.afnd.core.automata.AutomataGraph;
...
import com.cherrysoft.afnd.view.components.afnd.connections.VisualConnectionFactory;
import com.cherrysoft.afnd.view.components.afnd.states.*;
...
public class VisualAutomata extends AutomataGraph<String> implements Drawable {
  ...
  public VisualNode getVisualNodeBellow(Point point) {
    return (VisualNode) getNodes().stream()
        .filter(node -> {
          VisualNode visualNode = (VisualNode) node;
          return MathHelper.distanceBetweenTwoPoints(visualNode.getPos(), point) <= VisualNode.NODE_RADIUS;
        })
        .findAny()
        .orElse(null);
  }
